-- Importação administrativa de matrículas/vendas em lote.
-- A função é SECURITY DEFINER para manter as tabelas protegidas por RLS;
-- a autorização continua limitada a admin ou editor do projeto.
...
begin
  ...
  for v_row in select value from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    ...
    if nullif(trim(v_row->>'name'), '') is null or nullif(trim(v_row->>'sold_at'), '') is null then
    ...
    if v_email is not null or v_phone is not null then
      select id into v_contact_id from public.contacts
        where client_id = v_client_id and ((v_email is not null and normalized_email = v_email) or (v_phone is not null and normalized_phone = v_phone))
        limit 1;
    end if;
    ...
    values (v_contact_id, p_project_id, 'csv-lead-' || (v_row->>'external_sale_id'), v_row->>'sold_at', 'csv_import', 'CONVERTIDO', 'NONE', v_row->'raw_payload')
    ...
    values (v_contact_id, p_project_id, v_lead_event_id, v_seller_id, v_row->>'external_sale_id', nullif(v_row->>'amount', '')::numeric, coalesce(nullif(v_row->>'status', ''), 'PAID'), nullif(v_row->>'payment_method', ''), v_row->>'sold_at', v_row->'raw_payload')
