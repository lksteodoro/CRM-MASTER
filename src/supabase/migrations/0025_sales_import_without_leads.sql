-- A importação pela área de Vendas não cria lead_events.
-- Mantém os dados extras em raw_payload e cria o vendedor quando necessário.
delete from public.lead_events
 where source = 'csv_import' and external_id like 'csv-lead-%';
...
begin
  ...
  for v_row in select value from jsonb_array_elements(coalesce(p_rows, '[]'::jsonb)) loop
    ...
    v_seller_name := nullif(trim(v_row->>'seller_name'), '');
    v_seller_id := null;
    if v_seller_name is not null then
      ...
      if v_seller_id is null then
        insert into public.sellers (client_id, name, active) values (v_client_id, v_seller_name, true) returning id into v_seller_id;
      end if;
    end if;
    ...
    values (v_contact_id, p_project_id, null, v_seller_id, v_row->>'external_sale_id', nullif(v_row->>'amount', '')::numeric, coalesce(nullif(v_row->>'status', ''), 'PAID'), nullif(v_row->>'payment_method', ''), v_sold_at, v_row->'raw_payload')
