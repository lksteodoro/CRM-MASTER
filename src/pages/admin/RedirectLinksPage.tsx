...
function emptyForm(clientId = ''): RedirectLinkInput {
  return { client_id: clientId, name: '', slug: '', strategy: 'single', delay_seconds: 0, active: true, destinations: [{ label: 'Destino principal', target_url: '' }] };
}
              ...
              <div className="grid gap-3 lg:grid-cols-2">
                {clientLinks.map((link) => (
                  <article key={link.id} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-2 text-[10px] text-[var(--color-text-muted)]"><span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-panel-2)] px-2 py-1"><Clock3 size={11} /> {link.delay_seconds}s</span><span className="inline-flex items-center gap-1 rounded-full bg-[var(--color-panel-2)] px-2 py-1"><Repeat2 size={11} /> {link.strategy === 'round_robin' ? 'Loop' : 'Destino fixo'}</span><span className="rounded-full bg-[var(--color-panel-2)] px-2 py-1">{link.hit_count.toLocaleString('pt-BR')} acessos</span></div>
...
function RedirectLinkModal({ clients, link, initialClientId, onClose, onSaved }: { clients: ClientRow[]; link: RedirectLinkWithDestinations | null; initialClientId: string; onClose: () => void; onSaved: (link: RedirectLinkWithDestinations) => void }) {
  const [form, setForm] = useState<RedirectLinkInput>(() => link ? { client_id: link.client_id, name: link.name, slug: link.slug, strategy: link.strategy, delay_seconds: link.delay_seconds, active: link.active, destinations: link.destinations.map((destination) => ({ label: destination.label, target_url: destination.target_url })) } : emptyForm(initialClientId));
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4"><form onSubmit={(event) => void submit(event)} role="dialog" aria-modal="true" aria-labelledby="redirect-modal-title" className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-[var(--color-border)] bg-[var(--color-panel)] p-5 shadow-2xl sm:p-6"><div className="flex items-start justify-between"><div><h2 id="redirect-modal-title" className="font-semibold text-[var(--color-text)]">{link ? 'Editar redirecionador' : 'Novo redirecionador'}</h2><p className="text-xs text-[var(--color-text-muted)]">Organize o link, o tempo e a ordem dos destinos.</p></div><button type="button" onClick={onClose} className="rounded-lg p-1.5 text-[var(--color-text-muted)] hover:bg-[var(--color-panel-2)]" aria-label="Fechar"><X size={17} /></button></div>
    ...
    <div className="mt-3 grid gap-3 sm:grid-cols-2"><div><label className={labelClass}>Distribuição</label><select value={form.strategy} onChange={(event) => setForm((current) => ({ ...current, strategy: event.target.value as RedirectLinkInput['strategy'] }))} className={inputClass}><option value="single">Destino fixo</option><option value="round_robin">Loop / rodízio</option></select></div><div><label className={labelClass}>Tempo antes de redirecionar</label><div className="relative"><input type="number" min="0" max="300" step="1" value={form.delay_seconds} onChange={(event) => setForm((current) => ({ ...current, delay_seconds: event.target.valueAsNumber || 0 }))} className={`${inputClass} pr-16`} /><span className="absolute right-3 top-2.5 text-xs text-[var(--color-text-faint)]">segundos</span></div></div></div>
    <div className="mt-5"><div className="flex items-center justify-between"><div><h3 className="text-sm font-medium text-[var(--color-text)]">Destinos</h3><p className="text-[11px] text-[var(--color-text-muted)]">No modo loop, cada acesso usa o próximo destino e recomeça ao chegar no fim.</p></div><button type="button" onClick={() => setForm((current) => ({ ...current, destinations: [...current.destinations, { label: `Destino ${current.destinations.length + 1}`, target_url: '' }] }))} className="text-xs text-[var(--color-brand)] hover:underline">+ Adicionar destino</button></div><div className="mt-3 space-y-2">{form.destinations.map((destination, index) => <div key={index} className="grid gap-2 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg)] p-3 sm:grid-cols-[36px_0.7fr_1.5fr_auto]"><span className="flex h-9 w-9 items-center justify-center rounded-lg bg-[var(--color-brand-soft)] text-xs font-semibold text-[var(--color-brand)]">{index + 1}</span><input value={destination.label ?? ''} onChange={(event) => setForm((current) => ({ ...current, destinations: current.destinations.map((item, itemIndex) => itemIndex === index ? { ...item, label: event.target.value } : item) }))} className={inputClass} placeholder="Nome do destino" /><input type="url" value={destination.target_url} onChange={(event) => setForm((current) => ({ ...current, destinations: current.destinations.map((item, itemIndex) => itemIndex === index ? { ...item, target_url: event.target.value } : item) }))} className={inputClass} placeholder="https://destino.com/pagina" required />{form.destinations.length > 1 && <button type="button" onClick={() => setForm((current) => ({ ...current, destinations: current.destinations.filter((_, itemIndex) => itemIndex !== index) }))} className="rounded-lg p-2 text-[var(--color-bad)] hover:bg-[var(--color-bad-soft)]" aria-label={`Remover destino ${index + 1}`}><Trash2 size={14} /></button>}</div>)}</div></div>
